...
from managers.tag_extraction_manager import TagExtractionManager
...
class AppServices:
    def __init__(self, project_root: Path):
        ...
        self.replay_browser = BrowserManager(self.config)
        ...
        self.replay_page_manager = PageManager(self.config, self.classifier, self.replay_browser, self.persistence)
        self.tag_manager = TagManager(self.config, self.browser, self.page_manager, self.persistence, self.woo_api)
        self.tag_extractor = TagExtractionManager(project_root, self.browser, self.woo_api)
        self.run_manager = RunManager(self.config, self.browser, self.tag_manager, self.persistence, self.tag_extractor, replay_browser_manager=self.replay_browser)
    ...
    def reload_runtime(self) -> None:
        ...
        self.replay_browser.config = self.config
        self.page_manager.config = self.config
        self.replay_page_manager.config = self.config

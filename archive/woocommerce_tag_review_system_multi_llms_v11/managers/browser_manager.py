...
class BrowserManager:
    ...
    def open_page_fast(self, url: str):
        with self._lock:
            self.ensure_browser()
            timeout = self.config.get('runtime', {}).get('navigation_timeout_ms', 45000)
            pages = []
            ...
            if pages:
                opener = pages[-1]
                with self.context.expect_page(timeout=timeout) as page_info:
                    opener.evaluate("(targetUrl) => window.open(targetUrl, '_blank')", url)
    ...
    def has_live_browser_window(self) -> bool:
        with self._lock:

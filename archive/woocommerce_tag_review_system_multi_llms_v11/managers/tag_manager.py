...
class TagManager:
    ...
    def _build_page_url(self, tag_url: str, page_number: int) -> str:
        parsed = urlparse(tag_url or '')
        qs = parse_qs(parsed.query)
        qs['paged'] = [str(max(1, int(page_number or 1)))]
    ...
    def _process_tag_via_api(self, run_id: str, tag) -> Dict[str, Any]:
        ...
        execution_mode = self._execution_mode(tag)
        run_mode = self._run_mode(tag)
        start_page = self._page_number_from_url(tag.tag_url)

        if execution_mode == 'assessment_only':
            page_groups = self.woo_api_manager.fetch_products_for_tag_pages(
                int(resolved['id']),
                start_page=start_page,
                max_pages=(1 if run_mode == 'per_page' else tag.max_pages),
            )
            products = [product for page_group in page_groups for product in page_group.get('products', [])]
            append_log(tag.logs, 'info', 'Woo API products fetched for assessment-only mode', tag_id=resolved['id'], products=len(products), pages=len(page_groups), slug=resolved.get('slug'))
            ...
            append_log(tag.logs, 'info', 'Woo API classification map built', results=len(result_map), mode='assessment_only')
            for page_group in page_groups:
                page_number = int(page_group.get('page_number') or start_page)
                page_url = self._build_page_url(tag.tag_url, page_number)
                page_data = self.page_manager.process_api_page_without_browser(run_id, tag.to_dict(), page_number, page_url, page_group.get('products', []), result_map)
        ...
        append_log(tag.logs, 'info', 'Tag completed', status=tag.status, duration_seconds=tag.duration_seconds, carried_forward_count=tag.carried_forward_count, mode='woo_api', execution_mode=execution_mode)
        data = tag.to_dict()

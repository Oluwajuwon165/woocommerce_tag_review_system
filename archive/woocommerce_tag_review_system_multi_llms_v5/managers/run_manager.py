...
class RunManager:
    def __init__(self, config: Dict[str, Any], browser_manager, tag_manager, persistence_manager):
        self.config = config
    ...
    def _worker_loop(self) -> None:
        while True:
            run_data = self._queue.get()
            try:
                self._execute_run(run_data)
            finally:
                self._queue.task_done()

    def parse_tag_lines(self, text: str, strictness: str, max_pages: int, dry_run: bool) -> List[Dict[str, Any]]:
        tags = []

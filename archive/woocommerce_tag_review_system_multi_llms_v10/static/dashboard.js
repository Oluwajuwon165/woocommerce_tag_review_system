...
async function refreshRecentRuns() {
  ...
  if (!runs.length) {
    tbody.innerHTML = '<tr><td colspan="8">No runs yet.</td></tr>';
    return;
  }
  tbody.innerHTML = runs.map(run => `
    <tr>
      <td><code>${run.run_id}</code></td>
      <td>${(run.tags || []).map(tag => tag.tag_title).join(', ') || '—'}</td>
  ...
  startBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const payload = {
        ...
        execution_mode: document.querySelector('input[name="execution-mode"]:checked')?.value || 'full_review',

...
def api_runs_start():
    ...
    try:
        tags = services.run_manager.parse_tag_lines(tag_lines, strictness, max_pages, dry_run, run_mode)
...
def api_close_browser():
    ...
    return jsonify({'ok': True, 'browser_state': services.browser.browser_state()})
...
def api_replay_selected_page(run_id: str, tag_id: str, page_number: int):
    ...
    try:
        result = services.run_manager.call_in_worker(services.page_manager.replay_selected_assessments, run_id, tag, page, selected_products)

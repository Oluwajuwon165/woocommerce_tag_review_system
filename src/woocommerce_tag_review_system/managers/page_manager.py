...
class PageManager:
    ...
    def _execution_mode(self, tag_session: Dict[str, Any]) -> str:
        value = str((tag_session or {}).get('execution_mode') or '').strip().lower()
        return value if value in {'full_review', 'actionable_review', 'assessment_only'} else 'full_review'

    def _should_preserve_page(self, tag_session: Dict[str, Any], page_status: str) -> bool:
        mode = self._execution_mode(tag_session)
        if mode == 'assessment_only':
            return False
        if mode == 'actionable_review':
            return page_status == 'actionable'
        ...
        return preserve_all_pages or page_status == 'actionable' or (page_status == 'review_only' and preserve_review_only)
    ...
    def process_page_with_result_map(self, run_id: str, tag_session: Dict[str, Any], page_number: int, page_url: str, result_map: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            page_session.preserved_tab = self._should_preserve_page(tag_session, page_session.status)
    ...
    def process_api_page_without_browser(self, run_id: str, tag_session: Dict[str, Any], page_number: int, page_url: str, products: List[Dict[str, Any]], result_map: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        page_session = PageSession(page_number=page_number, page_url=page_url, status='running', started_at=utc_now_iso())
        threshold = self._strict_threshold(tag_session['strictness'])
        append_log(page_session.logs, 'info', 'Processing API page without browser', page_number=page_number, page_url=page_url, products=len(products), threshold=threshold)

        page_session.total_rows = len(products)
        page_session.helper_rows_count = 0
        page_session.candidate_rows = 0

        for index, product in enumerate(products or [], start=1):
            product_id = str(product.get('productId') or '').strip()
            product_title = str(product.get('productTitle') or '').strip()
            item = {
                'snapshotIndex': index,
                'rowDomId': f'post-{product_id}' if product_id else '',
                'productId': product_id,
                'productTitle': product_title,
                'titleFound': bool(product_title),
                'checkboxExists': True,
                'checkboxValue': product_id,
                'editLink': '',
                'rowPreview': product_title,
                'rowType': 'product_row',
            }
            diagnostic = self._build_diag(item, threshold)
            if not product_id:
                diagnostic['skip_reason'] = 'missing stable product identifier from API payload'
                page_session.skipped_rows_count += 1
                page_session.row_diagnostics.append(diagnostic)
                continue
            if not product_title:
                diagnostic['skip_reason'] = 'missing product title from API payload'
                page_session.skipped_rows_count += 1
                page_session.row_diagnostics.append(diagnostic)
                continue
            ...
            result = dict(result_map.get(product_id) or {})
            if not result:
                result = self.classifier.classify(tag_session['tag_title'], product_title, tag_session['strictness'])
                result.setdefault('carried_forward', False)
            ...
            if decision == 'keep':
                page_session.keep_count += 1
                diagnostic['checkbox_result'] = 'decision_keep'
            elif decision == 'mark':
                page_session.mark_count += 1
                if tag_session['dry_run']:
                    diagnostic['checkbox_result'] = 'dry_run'
                elif confidence >= threshold or diagnostic['classification_source'] == 'cache' or diagnostic['classification_source'] == 'carry_forward':
                    diagnostic['checkbox_result'] = 'deferred_replay_selected_default'
                else:
                    diagnostic['checkbox_result'] = 'below_threshold'
            else:
                page_session.review_count += 1
                diagnostic['checkbox_result'] = 'decision_review'

            page_session.assessments.append(self._make_record(tag_session, page_number, product_id, diagnostic['row_dom_id'], product_title, result, diagnostic['carried_forward']))
            page_session.assessed_rows += 1
            page_session.row_diagnostics.append(diagnostic)

        page_session.acp_bulk_ready = False
        page_session.ready_for_human = False
        page_session.preserved_tab = False
        page_session.browser_tab_id = None
        page_session.tab_focus_token = None
        page_session.checkbox_count = 0

        if page_session.mark_count > 0:
            page_session.status = 'actionable'
        elif page_session.review_count > 0:
            page_session.status = 'review_only'
        else:
        ...
        page_session.ended_at = utc_now_iso()
        page_session.duration_seconds = elapsed_seconds(page_session.started_at, page_session.ended_at)
        append_log(page_session.logs, 'info', 'Assessment-only API page complete', status=page_session.status, assessed_rows=page_session.assessed_rows, keep=page_session.keep_count, mark=page_session.mark_count, review=page_session.review_count, llm_calls=page_session.llm_call_count, cache_hits=page_session.cache_hit_count, carried_forward=page_session.carried_forward_count)
        data = page_session.to_dict()
        self.persistence_manager.save_page(run_id, tag_session['tag_id'], data)
        return data
    ...
    def replay_selected_assessments(self, run_id: str, tag_session: Dict[str, Any], page_session: Dict[str, Any], selected_products: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            page = self.browser_manager.open_page_fast(page_url)
        except Exception:
